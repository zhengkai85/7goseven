...
#define HTPlayerSrcName(file) [@"THPlayer.bundle" stringByAppendingPathComponent:file]
...
- (void)reduction:(UIView *)view
{
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        [UIView animateWithDuration:0.7f animations:^{
            //self.backView.alpha = 1;  zhengkai mark
        } completion:^(BOOL finished) {
            //            显示之后，3秒钟隐藏
            if (self.backView.alpha == 1.0) {
                [self removeHandleBackTime];
...
- (void)changeFrame{
    ...
    UIImage *img = [UIImage imageNamed:HTPlayerSrcName(@"pause")];
...
- (UIButton *)closeBtn{
    ...
    [_closeBtn addTarget:self action:@selector(colseTheVideo:) forControlEvents:UIControlEventTouchUpInside];
    [_closeBtn setImage:[UIImage imageNamed:HTPlayerSrcName(@"close")] forState:UIControlStateNormal];
    [_closeBtn setImage:[UIImage imageNamed:HTPlayerSrcName(@"close")] forState:UIControlStateSelected];
...
- (UIView *)backView{
    ...
    [self.playOrPauseBtn setImage:[UIImage imageNamed:HTPlayerSrcName(@"play")] forState:UIControlStateSelected];
    self.playOrPauseBtn.frame = CGRectMake((_backView.width - img.size.width)/2, (_backView.height - img.size.height)/2, img.size.width, img.size.height);
...
- (UIView *)bottomView{
    ...
    //    全屏按钮
    UIImage *img = [UIImage imageNamed:HTPlayerSrcName(@"fullscreen")];
    self.fullScreenBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.fullScreenBtn.showsTouchWhenHighlighted = YES;
    [self.fullScreenBtn addTarget:self action:@selector(fullScreenAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.fullScreenBtn setImage:img forState:UIControlStateNormal];
    [self.fullScreenBtn setImage:[UIImage imageNamed:HTPlayerSrcName(@"nonfullscreen")] forState:UIControlStateSelected];
    self.fullScreenBtn.frame = CGRectMake(_bottomView.width-img.size.width - 10, (_bottomView.height-img.size.height)/2,img.size.width ,img.size.height);
    ...
    [self.progressSlider setThumbImage:[UIImage imageNamed:HTPlayerSrcName(@"dot")] forState:UIControlStateNormal];
...
-(void)setFullScreenSelect:(BOOL)bState {  //zhengkai mark
    self.fullScreenBtn.selected = bState;
    [[NSNotificationCenter defaultCenter] postNotificationName:kHTPlayerFullScreenBtnNotificationKey
                                                        object:self.fullScreenBtn];
    _backView.alpha = 0;
}
