...
- (void)viewDidLoad
{
    ...
    [BMNet getBidCategoryBlock:^(id posts, NSInteger code, NSString *errorMsg) {
        if(code == 200) {
            NSArray *arrData = (NSArray*)posts[@"data"];
            for(NSDictionary *dic in arrData) {
                CollectionCategoryModel *model = [CollectionCategoryModel modelWithDictionary:dic];
                [self.dataSource addObject:model];
                NSArray *arrSecond = dic[@"BidSecond"];
                NSMutableArray *sDatas = [[NSMutableArray alloc] initWithCapacity:arrSecond.count];
                for(NSDictionary *dicSecond in arrSecond) {
                    SubCategoryModel *sMode = [SubCategoryModel modelWithDictionary:dicSecond];
                    [sDatas addObject:sMode];
                }
              [self.collectionDatas addObject:sDatas];
            }
...
{
    NSArray *arr = self.collectionDatas[section];
    return arr.count;
}
